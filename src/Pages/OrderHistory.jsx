...
const OrderHistory = () => {
    ...
    useEffect(() => {
        const loadAllOrders = async () => {
            try {
                setloading(true)
                let response = await fetch("https://billingsoftware-backend-production.up.railway.app/orders/all-orders", {
                    headers : {
                ...
                <tbody>
                    {orders.map((order, index) => (
                        <tr key={order.orderId} className={index % 2 == 0 ? "bg-gray-200" : "bg-white"}>
                            ...
                            <td className='py-3  px-2'>₹{order.grandTotal.toFixed(2)}</td>
                            <td className='py-3  px-2'>{order.paymentMethod}</td>
                            <td className='py-3  px-2'><span className={`rounded-md px-2 ${order.paymentDetails.status=='COMPLETED'? "bg-green-700" : "bg-yellow-500"}`}>{order.paymentDetails.status}</span></td>
